"""Generated metric queries for dashboard `sales-performance-dashboard-3`. DO NOT EDIT.
...
METRICS = [
    {"id": 'total_revenue', "title": 'Total Revenue (Jul–Aug)', "type": 'kpi', "sql": "SELECT SUM(FALLAMOUNT) AS value FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-07-01' AND PLANDATE <= '2026-08-28'", "options": {}},
    {"id": 'order_volume', "title": 'Order Volume', "type": 'kpi', "sql": "SELECT COUNT(DISTINCT FID) AS value FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-07-01' AND PLANDATE <= '2026-08-28'", "options": {}},
    {"id": 'order_qty', "title": 'Contracted Quantity', "type": 'kpi', "sql": "SELECT SUM(FQTY_ORIGIN) AS value FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-07-01' AND PLANDATE <= '2026-08-28'", "options": {}},
    {"id": 'active_customers', "title": 'Active Customers', "type": 'kpi', "sql": "SELECT COUNT(DISTINCT FCUSTID) AS value FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-07-01' AND PLANDATE <= '2026-08-28'", "options": {}},
    {"id": 'daily_revenue', "title": 'Daily Revenue (Aug)', "type": 'line', "sql": "SELECT DATE(PLANDATE) AS d, SUM(FALLAMOUNT) AS revenue FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-08-01' AND PLANDATE < '2026-09-01' GROUP BY DATE(PLANDATE) ORDER BY d", "options": {'delta': 8572.57, 'deltaLabel': 'vs prev. period', '_locked': {'current': 21716800.0, 'previous': 250407.824}}},
    {"id": 'monthly_trend', "title": 'Monthly Revenue (2026)', "type": 'bar', "sql": "SELECT DATE_FORMAT(PLANDATE, '%Y-%m') AS month, SUM(FALLAMOUNT) AS revenue FROM erp_v_sale_orderentry WHERE YEAR(PLANDATE) = 2026 GROUP BY DATE_FORMAT(PLANDATE, '%Y-%m') ORDER BY month", "options": {'topItem': {'label': '2026-04', 'value': 404934020.533, 'share_pct': 17.4}}},
    {"id": 'regional_split', "title": 'Regional Split (Aug)', "type": 'pie', "sql": "SELECT org_name AS region, SUM(FALLAMOUNT) AS revenue FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-08-01' AND PLANDATE < '2026-09-01' GROUP BY org_name", "options": {'topItem': {'label': '惠州伊斯科', 'value': 270624538.4828, 'share_pct': 83.6}}},
    {"id": 'top_products', "title": 'Top Products (Aug)', "type": 'bar', "sql": "SELECT material_name AS product, SUM(FALLAMOUNT) AS revenue FROM erp_v_sale_orderentry WHERE PLANDATE >= '2026-08-01' AND PLANDATE < '2026-09-01' GROUP BY material_name ORDER BY revenue DESC LIMIT 10", "options": {'topItem': {'label': '抽余碳五', 'value': 68360034.51, 'share_pct': 21.1}}},
]
