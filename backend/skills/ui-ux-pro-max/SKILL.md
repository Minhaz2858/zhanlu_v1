...
# /ui-ux-pro-max (Zhanlu Integration Wrapper)

> Skill-body wrapper for the Zhanlu global agent. Backed by two tool calls
> registered in the runtime: `uiux_search` (single-domain) and
> `uiux_design_system` (auto-aggregated full design spec).

## When to call

| Trigger | Why |
|---|---|
| Building a dashboard | Get a color palette, typography, chart types, UX checklist |
| Building any HTML artifact | Get the design tokens + UX pre-delivery checklist |
| User says "make this look professional / polished / premium" | Pull a style recommendation |
| Chart-heavy UI | Pull `domain=chart` for type + library recommendations |
| Mobile / responsive | Pull `domain=web` for iOS/Android/React Native rules |

**If the artifact is not visual (docx, pptx only-text, plain markdown) — skip this skill.**

## Invocation

### `uiux_search(query, domain?, stack?, max_results=3)`

| Arg | Type | Notes |
|---|---|---|
| `query` | str | Required. Free-text description of what you're building |
| `domain` | str | One of: `style`, `color`, `chart`, `landing`, `product`, `ux`, `typography`, `google-fonts`, `icons`, `gsap`, `react`, `web`. **Auto-detected if omitted.** |
| `stack` | str | One of 22: `html-tailwind` (default), `react`, `nextjs`, `vue`, `nuxtjs`, `svelte`, `astro`, `swiftui`, `react-native`, `flutter`, `shadcn`, `jetpack-compose`, `threejs`, `angular`, `laravel`, `javafx`, `wpf`, `winui`, `avalonia`, `uno`, `uwp`. |
| `max_results` | int | Default 3. Cap at 5 to stay token-budget friendly. |

Returns the markdown formatted by the upstream CLI (token-optimized, ~300 char
per cell unless `UNTRUNCATED_COLS`).

### `uiux_design_system(query, project?, variance?, motion?, density?)`

Generates a full design system spec (palette + typography + style + UX checklist)
in one call. Aggregates `product + style + color + typography + landing` domains
and applies the reasoning rules.

| Optional dial | Range | Effect |
|---|---|---|
| `variance` | 1-10 | 1=centered/minimal, 10=bold/asymmetric |
| `motion` | 1-10 | 1=subtle, 10=complex; attaches a GSAP snippet |
| `density` | 1-10 | 1=spacious, 10=dense/dashboard; overrides spacing scale |

## Recommended workflow

When the user asks for a dashboard / HTML artifact:

```
1. uiux_design_system(query="<topic>", stack="html-tailwind")
   → returns palette + typography + style + UX checklist (markdown)

2. uiux_search(query="<chart keyword>", domain="chart", stack="html-tailwind")
   → returns chart type recommendations with library snippets

3. uiux_search(query="<audience>", domain="color", max_results=2)
   → optional: refine palette if design-system output was generic

4. Then call build-dashboard skill (which now has design tokens to inject)
```

## Pre-delivery checklist (from `domain=ux`)
...
- Bright neon colors
- Harsh animations
- Dark mode by default for dashboards
